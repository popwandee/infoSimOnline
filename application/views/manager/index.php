...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function show_info(){
            $.ajax({
                type  : 'ajax',
                url   : '<?php echo site_url('mChecker')?>',
                async : true,
                dataType : 'json',
                success : function(data){
                    ...
                    for(i=0; i<data.length; i++){
                        ...
                        html += '<tr>'+
                                '<td>'+data[i].infoId+'</td>'+
                                '<td><a href="mInfoDetail/'+data[i].infoId+'">'+data[i].title+'</a></td>'+
                                ...
                                if (data[i].statusId < 3) {
                                    html +='<a href="mOperateInfo/'+data[i].infoId+'" class="btn btn-success btn-sm item_edit" data-infoId="'+data[i].infoId+'">แจ้งดำเนินการแล้ว</a>'
