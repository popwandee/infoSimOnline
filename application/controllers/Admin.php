...
class Admin extends BaseController
{
    ...
    function addNew()
    {
            ...
            $this->global['pageTitle'] = 'InfoSim : เพิ่มผู้ใช้ ';
    ...
    function addNewUser()
    {
            ...
            else
            {
                ...
                if($result > 0)
                {
                    ...
                    $this->session->set_flashdata('success', 'สร้างผู้ใช้ สำเร็จแล้ว');
                }
                else
                {
                    $this->session->set_flashdata('error', 'การสร้างผู้ใช้ ล้มเหลว');
                }
    ...
    function editOld($userId = NULL)
    {
            ...
            $this->global['pageTitle'] = 'InfoSim : แก้ไขผู้ใช้ ';
    ...
    function editUser()
    {
            ...
            else
            {
                ...
                if($result == true)
                {
                    ...
                    $this->session->set_flashdata('success', 'อัปเดตผู้ใช้ เรียบร้อยแล้ว');
                }
                else
                {
                    $this->session->set_flashdata('error', 'การอัพเดทผู้ใช้ ล้มเหลว');
                }
    ...
    {
            $data['dbinfo'] = $this->user_model->gettablemb('tbl_log','InfoSim');
    ...
    function backupLogTable()
    {
        ...
        if($backup)
        {
            $this->session->set_flashdata('success', 'Backup และการล้างตารางสำเร็จ');
            redirect('log-history');
        }
        else
        {
            $this->session->set_flashdata('error', 'Backup และการล้างตารางไม่สำเร็จ');
            redirect('log-history');
        }
    }
    ...
    {
            $data['dbinfo'] = $this->user_model->gettablemb('tbl_log_backup','InfoSim');
            if(isset($data['dbinfo']->total_size))
            {
            if(($data['dbinfo']->total_size)>1000){
                $this->backupLogTable();
    ...
    function backupLogTableDelete()
    {
        ...
        if($backup)
        {
            $this->session->set_flashdata('success', 'ล้างข้อมูลตารางสำเร็จ');
            redirect('log-history-backup');
        }
        else
        {
            $this->session->set_flashdata('error', 'การล้างตารางล้มเหลว');
            redirect('log-history-backup');
        }
    }
    ...
    function logHistoryUpload()
    {
            ...
            $process = 'อัปโหลดบันทึกการสำรองข้อมูล';
    ...
    function logHistoryUploadFile()
    {
        ...
        else if ($optioninput != '0' && $_FILES['filebackup']['name'] == '')
        {
            ...
            {
                $this->session->set_flashdata('error', 'การติดตั้งสำรองล้มเหลว');
                redirect('log-history-upload');
            }
            else
            {
                $this->session->set_flashdata('success', 'การติดตั้งสำรองสำเร็จ');
                redirect('log-history-upload');
            }
    }
}
