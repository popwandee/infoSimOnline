...
class Login extends BaseController
{
    ...
    public function noaccess() {
        ...
        $this->global['pageTitle'] = 'InfoSim : ปฏิเสธการเข้าใช้';
        $this->datas();
            ...
            if($this->login_model->checkEmailExist($email))
            {
                ...
                if($save)
                {
                    ...
                    if(!empty($userInfo)){
                        ...
                        $data1["message"] = "รีเซ็ตรหัสผ่าน";
                    }

                    $sendStatus = resetPasswordEmail($data1);

                    $process = 'ขอรีเซ็ตรหัสผ่าน';
                    $processFunction = 'Login/resetPasswordUser';
                    $this->logrecord($process,$processFunction);

                    if($sendStatus){
                        $status = "send";
                        setFlashData($status, "อัปโหลดรหัสผ่านของคุณเรียบร้อยแล้วตรวจสอบอีเมลของคุณ.");
                    } else {
                        $status = "notsend";
                        setFlashData($status, "การดำเนินการส่งอีเมลล้มเหลวโปรดลองอีกครั้ง.");
                    }
                }
                else
                {
                    $status = 'unable';
                    setFlashData($status, "เกิดข้อผิดพลาดในการส่งข้อมูลของคุณโปรดลองอีกครั้ง.");
                }
            }
            else
            {
